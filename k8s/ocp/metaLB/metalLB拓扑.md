...
Router A / B：企业边缘路由器，与 MetalLB 建立 eBGP 会话，接收服务 VIP 路由。
Switch：核心交换机，提供集群内部及节点到路由器的二层连通性（也可承担三层网关功能）。
防火墙：连接互联网与内部网络，执行 DNAT（入站）和 SNAT（出站）。
K8s 节点：运行 MetalLB Speaker（FRR）和业务 Pod。
...
MetalLB BGPPeer	两个：
- peerAddress 10.0.0.1 (Router A, AS64501)
- peerAddress 10.0.0.2 (Router B, AS64502)
MetalLB BGPAdvertisement	将 192.168.10.100/32 通告给两个邻居，aggregationLength: 32
路由器 ECMP	开启，保证多下一跳负载均衡
防火墙 DNAT	203.0.113.10:16211 → 192.168.10.100:8443；
203.0.113.10:16210 → 192.168.10.100:8080
...
两个 BGP 邻居 与 Kubernetes 集群节点建立 eBGP 会话。
入站：外网用户 → 防火墙 DNAT → 路由器（ECMP 负载均衡）→ 节点 → Service → Pod。
出站：Pod → 节点 SNAT → 路由器 → 防火墙 SNAT → 互联网。
关键设备：防火墙、核心交换机、BGP 路由器均清晰标注。
